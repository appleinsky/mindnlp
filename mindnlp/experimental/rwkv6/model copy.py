...
class RWKV_BLOCK(nn.Module):
    ...
    def __init__(self, block_w: dict, batch_size: int, n_embd: int, n_head: int, state: mindspore.Tensor, v_first: mindspore.Tensor, i: int):
        ...
        # 初始化注意力参数
        self.x_stack = nn.Parameter(ops.stack([block_w['att.x_r'],
                                        block_w['att.x_w'],
                                        block_w['att.x_k'],
                                        block_w['att.x_v'],
                                        block_w['att.x_a'],
        ...
        # 初始化变量
        self.sx = ops.zeros([self.batch_size, self.n_embd])
        self.sx_ = self.sx.unsqueeze(1).expand([self.batch_size, 6, self.n_embd])
        self.xr, self.xw, self.xk, self.xv, self.xa, self.xg = ops.unbind(self.sx_, dim=1)
        self.x = ops.zeros([self.batch_size, self.n_embd])
        self.x_ = self.x.unsqueeze(1)
    ...
    def time_mixing(self, x: mindspore.Tensor, v_first: mindspore.Tensor) -> mindspore.Tensor:
        ...
        self.sx = self.state_view_time_1 - x
        self.state_view_time_1 = x

        self.x = x        
        self.sx_ = self.x_ + self.sx_ * self.x_stack
